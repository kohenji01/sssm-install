<main>
    <div class="container">
        ...
        {if $DATA.checkResult.writable_success}
            ...
            <a href="{$smarty.server.SCRIPT_NAME}/Install" class="btn btn-secondary">{t}前へ{/t}</a>
            <a href="{$smarty.server.SCRIPT_NAME}/Install/sev_env_file" class="btn btn-primary">{t}次へ{/t}</a>
        {else}
            <p class="alert-danger">{t}すべてのチェックをクリアできていません。{/t}</p>
            <a href="{$smarty.server.SCRIPT_NAME}/Install/checkenv" class="btn btn-info">{t}再試行{/t}</a>
